...
def ease_out(u: float) -> float:
    """Cubic ease-out for dock morph and bubble fly (fast start, soft land)."""
    t = min(1.0, max(0.0, float(u)))
...
#: 50px raised resting position so the bar clears Reels chrome.
#: Scales 1:1 with ``preview_scale`` (350 → 175 at preview_scale=0.5).
_DOCK_CLEARANCE_PX = 350
...
_CTA_HOLD_S = 2.0
_CTA_TYPE_FRACTION = 0.28  # CTA reveals fully by ~28% of the VO window (faster)
_CTA_TTS_RATE = "+28%"  # edge-tts rate bump for a quicker spoken CTA
# Submit fly: slight opacity dip in place, then rise — slower than a snap.
_FLY_DURATION_S = 0.48
_FLY_FADE_FRAC = 0.34  # first ~34% of fly = fade-in-place; rest = rise
...
class Segment:
    ...
    def revealed_chars(self, t: float) -> int:
        ...
        if text_len <= 0:
            return 0
        # Use the same speech window as ``_typing_end_s``: hold ratio carves the
        # readable pause after the last spoken/typed character.
        typing_window = max(1e-6, 1.0 - self.typing_hold_ratio)
        local = max(0.0, float(t) - self.start_s)
        speech_s = max(1e-6, self.duration_s * typing_window)
...
class TerminalRenderer:
    ...
    def _draw_send_arrow(
        ...
        *,
        ...
        """Stroke-only up-arrow (chevron + stem), optically centred.

        No filled triangle — three strokes of equal width. Geometry is built
        around the visual centroid, then nudged slightly down/right (common
        optical correction for asymmetric arrow glyphs that otherwise look
        high-left when centred by bounding box alone).
        """
        x, y = origin
        w = max(14, int(size))
        h = max(16, int(size))
        # Optical nudge: ~6% of the glyph size down and right.
        nudge_x = max(1, int(round(w * 0.06)))
        nudge_y = max(1, int(round(h * 0.06)))
        cx = x + w // 2 + nudge_x
        cy = y + h // 2 + nudge_y
        stroke = max(3, int(round(w * 0.16)))
        # Chevron tip and shoulders (upper half).
        tip = (cx, cy - int(round(h * 0.38)))
        left = (cx - int(round(w * 0.40)), cy - int(round(h * 0.02)))
        right = (cx + int(round(w * 0.40)), cy - int(round(h * 0.02)))
        # Stem runs from just under the tip join down toward the base.
        stem_top = (cx, cy - int(round(h * 0.06)))
        stem_bot = (cx, cy + int(round(h * 0.40)))
        draw.line([left, tip, right], fill=fill, width=stroke, joint="curve")
        draw.line([stem_top, stem_bot], fill=fill, width=stroke)
    ...
    def _draw_block(
        ...
        *,
        ...
        accent = _GEMINI_BLUE if user_bubble else _ORANGE_CARET
        label_color = accent
    ...
    def _send_btn_size(self) -> int:
        ...
        return max(32, int(round(44 * self.config.preview_scale)))
    ...
    def _paste_flying_send(
        ...
        *,
        ...
        # (2) ease-out rise into the thread. Never fade and rise at once.
        ...
        if u <= fade_frac:
            ...
            rise_u = ease_out((u - fade_frac) / max(1e-6, 1.0 - fade_frac))
            text_opacity = 1.0 - float(_FLY_OPACITY_DIP) * (1.0 - rise_u)
        ...
        bubble_a = int(round(255 * _BUBBLE_OPACITY * max(rise_u, 0.20) * text_opacity))
        radius = max(12, min(22, int(18 * self.config.preview_scale)))
        ...
        draw.text((tag_x, tag_y), tag, font=self._font_small, fill=(*_GEMINI_BLUE, text_a))
    ...
    def _draw_compose_bar(
        ...
        *,
        ...
        size = max(14, int(btn * 0.52))
        self._draw_send_arrow(
            draw,
            (btn_left + (btn - size) // 2, btn_top + (btn - size) // 2),
            size=size,
            fill=_TITLE_WHITE,
        )
    ...
    def viewport_scroll_px(self, segments: Sequence[Segment], index: int, t: float, *, revealed: int | None = None) -> tuple[int, bool]:
        ...
        top of the chat mask. The viewport height matches the *docked* compose
        layout (not the full static body), so late-turn overflow is measured
        against the same mask the frame actually paints. Streaming responses
        re-evaluate height every revealed character and pin scroll to keep the
        typing edge on-screen — easing only applies to discrete new-message jumps.
        """
        layout = self._layout
        # Match ``_compose``: body shrinks to sit above the docked input bar.
        _, bar_top, _, _, _, _ = self._compose_geometry("", dock=1.0, anchor="bottom")
        ...
        # While a line is actively typing (compose draft OR target stream), shrink
        # the effective viewport so the live edge clears the input-bar mask.
        if composing or streaming:
            vh = max(1, vh - int(_STREAM_SCROLL_PAD_PX * max(0.5, float(self.config.preview_scale))))
        if composing:
            new_h = prev_h
        elif streaming:
        ...
        if streaming or scroll_s <= 0 or new_off <= prev_off:
            # Live typing / already-settled: pin to the content bottom every frame.
            scroll_px = new_off
        else:
            if current.utterance.role is SpeakerRole.ORCHESTRATOR:
    ...
    def render(
        ...
        *,
        ...
        def make_frame(t: float) -> np.ndarray:
            ...
            composing = in_box
            # Caret blink is independent of character append — keep pulsing while
            # the compose input is active (typing OR post-type hold before send).
            caret_on = (
            ...
            # Outro: drop the compose bar so the last bubble hands into the fade.
    ...
    def _draw_cta_frame(self, text: str, revealed: int, *, caret_on: bool) -> np.ndarray:
        """Paint the black CTA end-card: wordmark + wrapped typewriter follow line.

        Uses the same safe-width wrap path as chat bubbles (``_wrap_for_font``)
        so long invites never bleed past both screen edges.
        """
        from PIL import Image, ImageDraw  # noqa: PLC0415
        ...
        # Follow line: wrap inside ~78% of the content column, centre each line.
        lines = self._wrap_for_font(visible, font=self._font_compose, width_frac=0.78) if visible else [""]
        line_h = self._line_height_of(self._font_compose, scale=1.55)
        ly = wy + int(self.height * 0.10)
        last_lx, last_lw, last_ly, last_lh = self._layout.margin_x, 0, ly, line_h
        for line in lines:
            lbox = draw.textbbox((0, 0), line or " ", font=self._font_compose)
            lw = max(1, lbox[2] - lbox[0]) if line else 0
            lx = (self.width - max(1, lw)) // 2
            # Hard clamp against both edges.
            lx = max(self._layout.margin_x, min(lx, self.width - self._layout.margin_x - max(1, lw)))
            if line:
                draw.text((lx, ly), line, font=self._font_compose, fill=_CHAT_TEXT)
            last_lx, last_lw, last_ly, last_lh = lx, lw, ly, max(1, lbox[3] - lbox[1])
            ly += line_h
        if caret_on and revealed < len(text):
            cx = last_lx + last_lw
            draw.rectangle(
                [cx, last_ly, cx + max(2, self.width // 140), last_ly + last_lh],
                fill=accent,
            )
