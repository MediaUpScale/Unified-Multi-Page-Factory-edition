...
VISUAL_IDENTITY_PROFILES: dict[str, dict[str, Any]] = {
    "lofi_risograph_v1": {
        ...
        "technique": (
            ...
            "halftone texture with hard-edged color shading."
        ),
    ...
    "lofi_painterly_v1": {
        ...
        "mood": "Evoking a quiet, sorrowful mood.",
        "format": "Painted illustration, vertical 9:16 composition.",
        "characters": {
            "woman": (
...
def get_visual_identity_profile(
    ...
    from core.economic_reel_lofi.style_modules import get_style_profile_dict

    key = str(name or DEFAULT_VISUAL_IDENTITY_PROFILE).strip() or (
        DEFAULT_VISUAL_IDENTITY_PROFILE
    )
    styled = get_style_profile_dict(key)
    if styled and str(styled.get("name") or "") and key in {
        styled.get("name"),
        styled.get("style_id"),
        "riso_retro_flat_v4",
        DEFAULT_VISUAL_IDENTITY_PROFILE,
        ...
        # Live Dev style lives in style_modules — not this registry copy.
        if key in {
            "riso_retro_flat_v4",
            "style-riso_painting_retro_vintage",
            DEFAULT_VISUAL_IDENTITY_PROFILE,
