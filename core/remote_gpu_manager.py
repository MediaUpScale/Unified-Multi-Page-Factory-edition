...
Loads API-format JSON templates from ``infra/runpod/workflows/`` and exposes
...
from core.comfy_workflow_convert import convert_ui_to_api, is_api_format
...
def is_remote_gpu_enabled(media: str | None = None) -> bool:
    ...
    if media:
        try:
            from agents.mcp.model_api_flows import media_uses_remote_gpu  # noqa: PLC0415
...
def workflows_dir() -> Path:
    ...
    new = (root / "infra" / "runpod" / "workflows").resolve()
    old = (root / "remote_GPU_workflows").resolve()
...
def resolve_page_lora_config(
    ...
    *,
    ...
    url = None
    try:
        from agents.mcp.model_api_flows import _channel_lora_url  # noqa: PLC0415
...
class RemoteGPUImageAdapter:
    ...
    def generate(
        ...
        *,
        ...
        try:
            from agents.media.text_utils import safe_output_stem  # noqa: PLC0415
        except Exception:  # noqa: BLE001
...
def get_image_adapter(*args: Any, **kwargs: Any):
    ...
    ``agents.media.providers.image_provider.get_image_adapter``.
    ...
    """
    from agents.media.providers.image_provider import (  # noqa: PLC0415
