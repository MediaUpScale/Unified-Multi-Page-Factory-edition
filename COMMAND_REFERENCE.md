...
| `LONG_CAPTION_IMAGE` | Clean illustration + logo only; long FB-style caption |
...
`python main.py --page anna_protocol --post-type LONG_CAPTION_IMAGE --quantity 8 --premium-relay --model-api-flow hightier_together_elevenlabs`  
*// Generates 8 long-caption images for anna_protocol with Claude humanizer captions and Together FLUX Dev stills.*
...
`python -m VisualQA_Agent.main --channel master_mei --beat "A gaunt worker trapped in a CRT dungeon..." --model black-forest-labs/FLUX.1-schnell`  
...
`python -m VisualQA_Agent.main --channel master_mei --max-retries 4 --threshold 0.85 -v`  
...
`python main.py --page anna_protocol --post-type STANDARD_QUOTE --quantity 3 --avatar ON --format IMAGE_AVATAR --economic --model-api-flow hightier_together_elevenlabs`  
*// Generates 3 portrait images that composite the anna_protocol likeness reference into the scene (Together FLUX Dev).*
...
*// Bypasses the avatar pipeline — purely atmospheric SMART_BAIT backgrounds (SMART_BAIT / LONG_CAPTION_IMAGE / CAROUSEL already force avatar OFF internally).*
...
`python -m facebook_scheduler.reels_scheduler --channel master_mei --cdp 56076 --max 10`  
*// Runs the Facebook Reels scheduler for master_mei using CDP port 56076, processing a maximum of 10 clips from the queue.*

`python -m facebook_scheduler.reels_scheduler --channel ancient_knowledge --cdp 9222 --max 5`  
*// Schedules up to 5 pending ancient_knowledge MP4s via the dedicated reels_composer endpoint.*

`python -m facebook_scheduler.reels_scheduler --channel wonder_feed --dry-run`  
...
`python -m facebook_scheduler.reels_scheduler --channel master_mei --max 3 --no-move`  
...
`python -m facebook_scheduler.reels_scheduler --channel momma_circle`  
...
There is no `youtube_scheduler.shorts_scheduler` module. Use `publish_existing.py` for already-rendered MP4s, or `main.py --publish-youtube` (section 1.2) to generate-and-upload in one shot. Tokens: `credentials/tokens/youtube_token_{page}.json`.

`python publish_existing.py --page ancient_knowledge --video outputs/ancient_knowledge/clips/reel_example_v01.mp4 --schedule --interval 6`  
...
`python publish_existing.py --page ancient_knowledge --video reel_v01.mp4 reel_v02.mp4 reel_v03.mp4 --schedule`  
*// Uploads and drip-schedules three Shorts for ancient_knowledge (privacyStatus=private + publishAt).*

`python publish_existing.py --page master_mei --video outputs/master_mei/clips/reel_example_v01.mp4 --publish-at "2026-08-13 14:00"`  
*// Schedules a single Master Mei Short at an explicit UTC datetime (YYYY-MM-DD HH:MM).*

`python publish_existing.py --page wonder_feed --video outputs/wonder_feed/clips/reel_example_v01.mp4 --privacy unlisted --dry-run`  
*// Previews an immediate unlisted upload without calling the YouTube API.*

`python publish_existing.py --page ancient_knowledge --resume-youtube-queue`  
...
`python publish_existing.py --resume-youtube-queue`  
...
`python publish_existing.py --page ancient_knowledge --update-only --video-id VIDEO_ID --title "Corrected Title"`  
...
`python pinterest_main.py sync --channel anna_protocol --limit 50`  
...
`python pinterest_main.py sync --channel wonder_feed --no-ai --dry-run`  
*// Preview inventory sync using fast regex/templates instead of Claude; no writes.*

`python pinterest_main.py sync --channel anna_protocol --force --ai-delay 0.8`  
*// Re-generates Pinterest metadata even when fields already exist, with 0.8 s between Claude calls.*

`python pinterest_main.py transform --channel anna_protocol --method blurred_padding`  
*// Builds a 2:3 sales pin (1000×1500) for the first unposted inventory entry. Methods: blurred_padding (default), center_crop.*

`python pinterest_main.py transform --channel anna_protocol --post-id POST_ID --method center_crop --open`  
...
`python pinterest_main.py export --channel anna_protocol --posts-per-day 3 --slots 08:00 13:00 18:00 --start 2026-08-13 --mark-exported`  
...
*// Legacy library repair: preview Pinterest title/caption/visual_hook injection for up to 20 posts (no writes). Prefer pinterest_main.py sync for channel-scoped runs.*
...
*// Loads ComfyUI workflow JSONs under remote_GPU_workflows/, prints node counts and config, and makes no HTTP calls.*

`python test_remote_gpu.py --image --base-url http://127.0.0.1:8188 --mode comfyui`  
*// Live Flux Dev txt2img smoke test against a local/pod ComfyUI. Saves under outputs/remote_gpu_tests/.*

`python test_remote_gpu.py --audio --ref-audio path/to/voice.wav --base-url http://<POD_IP>:8188`  
*// Live F5-TTS txt2audio test, uploading the reference WAV to ComfyUI input/.*

`python test_remote_gpu.py --video --image-file path/to/frame.png --duration 5 --video-size 512`  
*// Live Wan2.2 img2vid test from a local still (required unless --dry-run).*

`python test_remote_gpu.py --all --base-url http://<POD_IP>:8188 --mode runpod --timeout 600`  
*// Runs image + audio + video remote-GPU tests on RunPod with a 600 s job timeout.*

`python test_audio_engine.py --page master_mei`  
...
| `--img-production` | — | — | Image provider/model override |
