...
# ---------------------------------------------------------------------------
_DEFAULT_CREDS = str(
    Path(__file__).resolve().parents[1] / "credentials" / "gsheet_service_account.json"
)
...
class SheetQueue:
    ...
    def from_config(cls) -> "SheetQueue":
        """Build from facebook_scheduler.config values (for main.py)."""
