...
Exercises each template under ``infra/runpod/workflows/`` independently and
...
--------
    ...
    python tests/test_remote_gpu.py --image --base-url http://127.0.0.1:8188

    # All three workflows
    python tests/test_remote_gpu.py --all --base-url http://<POD_IP>:8188

    # Audio with a local reference voice sample
    python tests/test_remote_gpu.py --audio --ref-audio path/to/voice.wav

    # Video from a local still
    python tests/test_remote_gpu.py --video --image-file path/to/frame.png
"""
...
def _build_manager(args: argparse.Namespace):
    from core.remote_gpu_manager import ComfyUIClient, RemoteGPUManager, reset_manager
