...
_TTS_WIRE_PATCHED = False


def _voice_settings_speed(vs: Any) -> Any:
    if vs is None:
        return None
    if isinstance(vs, dict):
        return vs.get("speed")
    return getattr(vs, "speed", None)


def _log_tts_wire_payload(layer: str, path_or_url: Any, payload: Any) -> None:
    """Print voice_settings.speed from the JSON that actually leaves the process."""
    blob = str(path_or_url or "")
    if "text-to-speech" not in blob:
        return
    body = payload
    ...
    if not isinstance(body, dict):
        return
    vs = body.get("voice_settings")
    speed = _voice_settings_speed(vs)
    if isinstance(vs, dict):
        vs_keys = sorted(vs.keys())
    elif vs is not None and hasattr(vs, "model_dump"):
        try:
            vs_keys = sorted((vs.model_dump(exclude_none=True) or {}).keys())
        except Exception:  # noqa: BLE001
            vs_keys = ["speed"]
    else:
        vs_keys = []
    text = body.get("text")
    preview = text if not isinstance(text, str) else (text[:80] + ("…" if len(text) > 80 else ""))
    msg = (
        f"[ElevenLabs TTS] WIRE {layer} path={blob} "
        f"model_id={body.get('model_id')!r} "
        f"voice_settings.speed={speed!r} "
...
def _ensure_tts_wire_logger() -> None:
    """Patch SDK HttpClient + httpx so every TTS call logs the outgoing body."""
    global _TTS_WIRE_PATCHED
    if _TTS_WIRE_PATCHED:
        return
    try:
        import httpx
    ...
    orig_http = HttpClient.request

    def _http_request(self, path=None, *args, **kwargs):
        try:
            _log_tts_wire_payload("HttpClient.json", path, kwargs.get("json"))
        except Exception:  # noqa: BLE001
            logger.debug("TTS wire HttpClient log failed", exc_info=True)
        return orig_http(self, path, *args, **kwargs)

    HttpClient.request = _http_request  # type: ignore[method-assign]

    orig_hx = httpx.Client.request

    def _hx_request(self, method, url, *args, **kwargs):
        try:
            payload = kwargs.get("json")
            if payload is None:
                payload = kwargs.get("content")
            _log_tts_wire_payload("httpx", url, payload)
        except Exception:  # noqa: BLE001
            logger.debug("TTS wire httpx log failed", exc_info=True)
        return orig_hx(self, method, url, *args, **kwargs)
...
def _resolve_elevenlabs_api_key() -> str:
    ...
    ``import config`` can bind quality/VisualQA_Agent/config.py or
    agents/posting/facebook_scheduler/config.py when those dirs shadow the factory root.
...
def generate_voiceover(
    ...
    *,
    ...
    print(
        ...
        f"intended_speed={_speed!r} serialized_speed={vs_dump.get('speed')!r} "
        f"fallback_if_unset={_NARRATION_SPEED} voice_settings={vs_dump} "
        f"chars={len(text)}"
    )
...
def generate_dynamic_music_prompt(
    ...
    *,
    ...
    # (see core/channel_rag_bridge.py gap docstring).
    _rag_music_block = ""
    try:
        from agents.rag.channel_rag import get_music_guidance  # noqa: PLC0415
