...
def apply_text_overlay(
    ...
    *,
    ...
    """
    # LONG_CAPTION_IMAGE / CTA_CAPTION_IMAGE / ECONOMIC_REEL: illustration must stay 100% clean.
    # LONG_CAPTION_IMAGE / CTA_CAPTION_IMAGE — text belongs in the post caption only, not on the image.
    ...
    if post_type in ("LONG_CAPTION_IMAGE", "ECONOMIC_REEL", "WAN_REEL", "CTA_CAPTION_IMAGE"):
        logger.info(
    ...
    try:
        ...
        final_rgba.convert("RGB").save(str(output_path), "PNG", compress_level=1)
...
def apply_logo_watermark(
    ...
    *,
    ...
    try:
        ...
        result.convert("RGB").save(str(output_path), "PNG", compress_level=1)
        logger.info(
...
def generate_text_quote_background(
    ...
    *,
    ...
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    bg.save(str(output_path), "PNG", compress_level=1)
