...
Terminology (keep distinct — never interchangeable):
  ...
  - *comfy_workflow* / workflow JSON — ``.json`` graphs under ``infra/runpod/workflows/``,
...
Architecture rule (keep it this way):
  ...
  ``infra/runpod/workflows/``. Adding a new ``.json`` under that folder does **not**
...
from core.remote_gpu_manager import (  # noqa: E402
...
def _parse_media_override(raw: str | None, *, kind: str) -> MediaResolved | None:
    ...
    if kind == "image":
        ...
        if provider in ("gemini", "google", "nano_banana"):
            model = rest or GEMINI_PRO_IMAGE
            if model.lower() in ("flash", "cheap", "flash-image"):
                model = GEMINI_FLASH_IMAGE
            elif model.lower() in ("pro", "preview"):
                model = GEMINI_PRO_IMAGE
            if not model.lower().startswith("models/") and "gemini" in model.lower():
                model = f"models/{model.removeprefix('models/')}"
            return MediaResolved(provider="gemini", model=model)
        ...
        raise ValueError(
            ...
            "Use gemini[/model], together[/model], or remote_gpu[/comfy_workflow.json]."
        )
...
def resolve_effective_lora(
    ...
    *,
    ...
    """
    from core.remote_gpu_manager import resolve_page_lora_config
    ...
    if name is None or str(name).strip().lower() in disabled:
        ...
        enabled = True
        from core.remote_gpu_manager import _normalize_lora_filename
