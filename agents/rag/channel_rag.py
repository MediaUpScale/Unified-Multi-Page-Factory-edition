...
-------------------------
* Script / voiceover agent  ->  core.reel_sequence_engine
                                :func:`build_sequence_script_prompt`
* Image-prompt agent        ->  main.py per-act AK loop (also uses
                                the pool planner in
                                ``agents.media.prompt_alignment``)
* Video / motion agent      ->  core.reel_sequence_engine
                                :func:`compile_sequence_reel`
* Music-prompt agent        ->  agents.media.audio_engine
...
def _load_rules(channel_name: str) -> "dict[str, Any] | None":
    ...
    if not name:
        return None
    try:
        from quality.VisualQA_Agent.channel_rag import get_channel_rules  # noqa: PLC0415
