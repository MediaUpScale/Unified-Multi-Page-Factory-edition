...
def reset_script_llm_call_log() -> None:
    ...
    from agents.mcp.text_model import reset_call_log as _reset_llm

    _reset_llm()
    from agents.rag.lofi_rag import reset_call_log as _reset_rag
...
def get_script_llm_call_log() -> list[dict[str, Any]]:
    ...
    RAG lookups are excluded here so episode ``script_llm_calls`` stays an
    LLM count. Use :func:`get_script_cost_log` for LLM + RAG together.
    """
    from agents.mcp.text_model import get_call_log as _llm_log

    llm = [e for e in _llm_log() if e.get("kind", "llm") == "llm"]
    return llm or list(_SCRIPT_LLM_CALLS)


def get_script_cost_log() -> list[dict[str, Any]]:
    """LLM completions plus RAG lookups for one episode."""
    from agents.mcp.text_model import get_call_log as _llm_log
    from agents.rag.lofi_rag import get_call_log as _rag_log

    return list(_llm_log()) + list(_rag_log())


def _estimate_tokens(text: str) -> int:
    """Rough chars/4 heuristic — same convention as core.cost_tracker."""
...
def _call_claude(prompt: str) -> str:
    ...
    return complete_script(prompt, provider="claude").text


def _call_gemini(prompt: str) -> str:
    from agents.mcp.text_model import complete_script

    return complete_script(prompt, provider="gemini").text
...
def _call_deepseek(prompt: str) -> str:
    ...
    return complete_script(prompt, provider="deepseek").text


def _call_llm(prompt: str) -> str:
    """DeepSeek V4 Flash first; Gemini only if DeepSeek is unavailable. Not Claude.

    Provider swap: set ``LOFI_SCRIPT_MODEL=deepseek|gemini|claude``.
    """
    from agents.mcp.text_model import complete_script

    return complete_script(prompt).text
...
def _finalize_thematic_narrative(
    ...
    *,
    ...
    from core.economic_reel_lofi.setting_archetypes import ARCHETYPE_ORDER
    ...
    for i, row in enumerate(lines or []):
        ...
        if row.get("intentional_objectless"):
            rec = {
                "scene": i + 1,
                "object": obj,
                "condition": "intentional_objectless",
                "caption": text,
            }
            report.append(rec)
            print(
                f"[LOFI object-continuity] scene={i + 1} object={obj!r} "
                f"condition=intentional_objectless caption={text!r}"
            )
            continue
        ...
        if (
            not caption_named
            and prior
            and _OBJECT_ANAPHORA_RE.search(blob)
        ):
            caption_named = True
        if caption_named:
            cond = "caption_named"
...
def generate_script(
    *,
    ...
    from agents.rag.lofi_rag import retrieve_script_seed

    rag_pack = retrieve_script_seed(
        module=module,
        theme=theme,
        subtheme=subtheme,
        theme_row=theme_row,
        force_hook_type=force_hook_type,
        thematic_hook_fn=_draw_thematic_hook_type,
        default_hook_fn=_draw_hook_type,
    )
    arc = rag_pack.arc
    thematic = rag_pack.thematic
    seed = rag_pack.seed
    retrieved = list(rag_pack.details)
    hook_type = rag_pack.hook_type
    quote = rag_pack.quote
    seed["quote"] = quote
...
"""
    ...
    if thematic:
        ...
        if not reasons and force_polish and not skip_polish:
            try:
                from agents.writer.claude_polish import polish_one
