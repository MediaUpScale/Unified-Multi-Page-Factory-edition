...
_NO_SIGNATURE_RULE = (
    "- Never append a copyright line, © notice, trademark, or 'by MediaUpScale' signature.\n"
)

_SIGNATURE_LINE_RE = re.compile(
    r"^\s*(?:\u00a9|copyright|\(c\))\b.+$|^\s*.*\bby MediaUpScale\s*$",
    re.IGNORECASE | re.MULTILINE,
)


def _strip_caption_signature(caption: str) -> str:
    """Drop footer signature lines the original Anna engine never used."""
    kept = [line for line in (caption or "").splitlines() if not _SIGNATURE_LINE_RE.search(line)]
    return "\n".join(kept).rstrip()


def _build_voice_rules(cta_enabled: bool = True, *, forbid_signature: bool = False) -> str:
    ...
    return (
        ...
        + ending
        + (_NO_SIGNATURE_RULE if forbid_signature else "")
...
def _pick_format_rules(cta_enabled: bool = True, *, forbid_signature: bool = False) -> str:
    """Return voice + format block, randomly short (60 %) or long (40 %)."""
    voice = _build_voice_rules(cta_enabled, forbid_signature=forbid_signature)
...
def build_claude_humanizer_user_prompt(
    ...
    *,
    ...
    channel=None,
    cta_caption_format: bool = False,
) -> str:
    format_rules = _pick_format_rules(
        cta_enabled, forbid_signature=cta_caption_format
    )
    if _is_isolated_channel(channel) and not cta_caption_format:
        format_rules = _active_channel(channel).get_system_prompt()
    suffix = _variant_suffix(variation_index, total_variants)
    tail = f"\n\nTopic focus: `{topic}`\n\nFACT SHEET:\n```\n{raw_fact_sheet}\n```"
    if cta_enabled:
        kw = cta_keyword or (
            contextual_cta_keyword(topic) if cta_caption_format else _cta_for(topic, channel)
        )
        cta_instruction = (
            CTA_VOICE_INSTRUCTION
            or f"Weave 'Comment {kw}' naturally into the caption as a personal DM invitation."
        )
        if _is_isolated_channel(channel) and not cta_caption_format:
            cta_instruction = _active_channel(channel).get_system_prompt()
        body = (
            f"{format_rules}\n"
...
def build_gemini_humanizer_instruction(
    ...
    *,
    ...
    format_rules = (
        ...
        if (_is_isolated_channel(channel) and not cta_caption_format)
        else _pick_format_rules(cta_enabled, forbid_signature=cta_caption_format)
    )
    suffix = _variant_suffix(variation_index, total_variants)
    if cta_enabled:
        kw = cta_keyword or (
...
class CaptionEngine:
    ...
    def _deepseek_complete(
        ...
        *,
        ...
        self._log_script_cost(provider="deepseek", model=_model, prompt=user_prompt, output=text)
        return text

    def _log_script_cost(self, *, provider: str, model: str, prompt: str, output: str) -> None:
        try:
            from agents.mcp.text_model import log_text_call

            log_text_call(provider=provider, model=model, prompt=prompt, output=output)
        except Exception:  # noqa: BLE001
            logger.debug("script cost log skipped", exc_info=True)
    ...
    def _gemini_text_complete(self, contents: str, *, model_id: str | None = None) -> str:
        ...
        from agents.media.providers.gemini_utils import chain_with_preferred_first
    ...
    def humanize_voice(
        ...
        *,
        ...
        user_prompt = build_claude_humanizer_user_prompt(
            ...
            cta_caption_format=cta_caption_format,
        )
    ...
    def generate_sequence_voiceover(
        ...
        *,
        ...
        """
        from core.reel_sequence_engine import build_sequence_script_prompt  # local import avoids circular
    ...
    def humanize_smart_bait(
        ...
        *,
        ...
        from agents.posting.post_planner import extract_clean_caption  # noqa: PLC0415
    ...
    def humanize_voice_with_fallback(
        ...
        *,
        ...
        if cta_enabled:
            kw = cta_keyword or (
                contextual_cta_keyword(topic)
                if cta_caption_format
                else _cta_for(topic, self._channel)
            )
        else:
            kw = None

        def _gemini_call(sheet: str) -> str:
            return self.humanize_voice_gemini(
                sheet, topic,
                variation_index=variation_index,
                total_variants=total_variants,
                model_id=(
                    (model_id or app_config.GEMINI_ECONOMIC_BRAIN_MODEL)
                    if economic
                    else model_id
                ),
                cta_keyword=kw,
        ...
        def _finish(text: str, tag: str) -> tuple[str, str]:
            cleaned = _strip_caption_signature(text) if cta_caption_format else text
            if cta_caption_format and cleaned:
                n = len(cleaned)
                logger.info(
                    "CTA_CAPTION_IMAGE | chars=%d | keyword=%s | tag=%s",
                    n, kw or "(off)", tag,
                )
            return cleaned, tag
        ...
        if economic:
            ...
            try:
                result = _gemini_call(raw_fact_sheet)
                ...
                if cta_caption_format and result:
                    n = len(result.strip())
                    if n < 300 or n > 850:
                        result = _gemini_call(
                            raw_fact_sheet
                            + "\n\nCRITICAL: Rewrite to the FORMAT length — "
                            "SHORT = one paragraph 350-450 characters; "
                            "LONG = 3-4 short paragraphs 550-750 characters. "
                            "No copyright footer."
                        ) or result
                if result:
                    logger.info("humanize_voice_with_fallback | Gemini PRIMARY OK")
                    return _finish(result, "humanized")
            except Exception as exc:  # noqa: BLE001
                logger.warning(
    ...
    def humanize_cta_caption(
        ...
        *,
        ...
        """
        Original Anna Holistic Legacy caption: 60% short (350-450 chars, one
        paragraph) or 40% long (550-750 chars, 3-4 short paragraphs), ending
        with ``Comment KEYWORD`` from Anna's DNA inventory. No copyright footer.
        """
        kw = (cta_keyword or contextual_cta_keyword(topic)) if cta_enabled else None
        return self.humanize_voice_with_fallback(
            raw_fact_sheet,
            topic,
            variation_index=variation_index,
            total_variants=total_variants,
