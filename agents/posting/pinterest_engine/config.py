...
Runtime root:
    Path(__file__).resolve().parents[3]  (factory root)
...
# Factory root (agents/posting/pinterest_engine/config.py → parents[3])
ENGINE_ROOT: Path = Path(__file__).resolve().parents[3]
...
def resolve_channel_env_path(channel_id: str) -> Path:
    ...
    Order:
      1. ``channels_config/<channel_id>/.env``
      2. ``channels_config/<channel_id>/.env.<channel_id>``
      3. ``.env.<channel_id>`` (legacy workspace root)
      4. workspace ``.env``
    """
    cid = _safe_channel_id(channel_id)
    candidates = [
        CHANNELS_CONFIG_DIR / cid / ".env",
        CHANNELS_CONFIG_DIR / cid / f".env.{cid}",
