...
python agents/posting/publish_existing.py \\
...
# ``agents.media.*`` modules resolve correctly when the script is run from
...
# Load factory .env so YOUTUBE_* env vars, etc. are available.
try:
...
def publish_videos(
    ...
    if publish_at_override:
        ...
        log.info(
            "Smart scheduler | first slot: %s UTC  interval: %dh  videos: %d  safety_cap=%d",
        ...
        print(
            ...
            f"(every {interval_hours} h) | safety cap={gate.limit}"
        )
    ...
    for idx, vpath in enumerate(video_paths, 1):
        if not gate.can_upload():
            gate.notify_halt()
            print(
                f"[YouTube] Queuing remaining {len(video_paths) - idx + 1} video(s) → "
            ...
            for _remaining_vpath in video_paths[idx - 1:]:
                ...
                _r_title = (
                    title_template or _r_lib.get("title") or _title_from_stem(_remaining_vpath.stem)
                )[:100]
                if not dry_run:
                    queue_pending_upload(
                        page_name=page,
                ...
                results.append({
                    "file": str(_remaining_vpath),
                    "status": "deferred",
                    "error": f"deferred by global safety cap ({gate.limit})",
                })
                if current_slot:
...
def _print_summary(results: list[dict], dry_run: bool) -> None:
    ...
    for r in deferred:
        fn = Path(r["file"]).name
        print(f"  [DEFER] {fn}")
        print(f"          {r.get('error', 'deferred by global safety cap')}")
        print()
    ...
    print(
        ...
        f"Deferred: {len(deferred)} | Failed: {len(err)}"
    )
    print(sep)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        metavar="N",
        help=(
            f"Global safety cap for successful uploads this run "
            f"(default: {MAX_DAILY_UPLOADS}). Remaining videos stay queued for the next run."
        ),
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"  Safety cap    : {args.limit if args.limit is not None else MAX_DAILY_UPLOADS}")
    if args.dry_run:
        print("  *** DRY RUN — no API calls will be made ***")
    print("=" * 68)
